...
// 1) sidebar collapse + reopen
let p = await page();
await open(p, 'overview');
await p.locator('aside button[aria-label*="접기"]').click(); await p.waitForTimeout(400);
await shot(p, 'sidebar_collapsed');
// 2) policy modal (re-expand first)
await p.locator('aside button[aria-label*="펼치"], aside button[aria-label*="접기"]').first().click(); await p.waitForTimeout(400);
await p.locator('aside').getByText('정책 안내').click(); await p.waitForTimeout(400);
await shot(p, 'policy_modal');
await p.keyboard.press('Escape'); await p.waitForTimeout(300);
// 3) user menu
await p.locator('aside').getByText('김삼성').click(); await p.waitForTimeout(400);
await shot(p, 'user_menu', { clip: { x: 0, y: 800, width: 500, height: 500 } });
