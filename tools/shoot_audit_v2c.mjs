...
mkdirSync('design/shots/audit_v2_final', { recursive: true });
...
async function el(p, sel, out) {
  ...
  await e.screenshot({ path: `design/shots/audit_v2_final/${out}.png` });
...
if (await filterBtn.count()) { await filterBtn.click(); await p.waitForTimeout(500);
  await p.screenshot({ path: 'design/shots/audit_v2_final/r_res_filter_open.png', clip: { x: 1100, y: 80, width: 820, height: 360 } });
...
if (await more.count()) { await more.click(); await p.waitForTimeout(800);
  await p.screenshot({ path: 'design/shots/audit_v2_final/r_tok_modal.png' });
